...
async fn main() {
    ...
    let ctx = connect_to_db(&host, &db_name);
...
fn connect_to_db(host: &str, db_name: &str) -> DbConnection {
    let db_name = db_name.to_string();
    let db_name_clone = db_name.clone();
    DbConnection::builder()
        // Register our `on_connect` callback, which will save our auth token.
        .on_connect(move |ctx, identity, token| on_connected(ctx, identity, token, &db_name))
        ...
        .with_token(
            creds_store(&db_name_clone)
                .load()
                .expect("Error loading credentials"),
        )
        // Set the database name we chose when we called `spacetime publish`.
        .with_module_name(&db_name_clone)
...
fn on_connected(_ctx: &DbConnection, _identity: Identity, token: &str, db_name: &str) {
    ...
    if let Err(e) = creds_store(db_name).save(token) {
